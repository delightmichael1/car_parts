...
export default function MagicRings({
  ...
  colorTwo = "#42fcff",
  speed = 1,
  ringCount = 6, // hardcoded shader loop caps this at 10 — GLSL loop bounds must be compile-time constants
  attenuation = 10,
  lineThickness = 2,
  baseRadius = 0.35,
  radiusStep = 0.1,
  ...
  useEffect(() => {
    ...
    if (!context) {
      // Deferred so the state update isn't synchronous inside this effect.
      queueMicrotask(() => setIsWebglSupported(false));
      return;
    }
    ...
    const resize = () => {
      ...
      // Skip until the container actually has a size — otherwise
      // uResolution ends up (0, 0) and 1.0 / min(uResolution.xy) in the
      // shader divides by zero, producing NaN/Inf fragments.
      if (w === 0 || h === 0) return;
    ...
    const animate = (t: number) => {
      ...
      uniforms.uRingCount.value = Math.round(p.ringCount);
  ...
  if (!isWebglSupported) {
    return (
      <div
        ref={mountRef}
        className="magic-rings-container h-60 w-60"
        aria-hidden="true"
        style={{
          background:
            "radial-gradient(circle at center, rgba(168, 85, 247, 0.22), transparent 65%)",
        }}
      />
    );
  }
  ...
  return (
    <div
      ref={mountRef}
      className="magic-rings-container h-full w-full"
